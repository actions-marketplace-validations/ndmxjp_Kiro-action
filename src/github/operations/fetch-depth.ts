...
/**
 * Builds the `--depth` argument for a `git fetch`, unless the checkout still has
 * its full history.
 *
 ...
 * with the base branch: `git log origin/<base>..HEAD` then quietly lists commits
 * that are already merged, and `git diff origin/<base>...HEAD` fails with "no
 * merge base". Those are the commands the prompt tells Kiro to run to scope its
 * work to the PR.
 *
 * A shallow checkout (the `fetch-depth: 1` default) has no history left to lose,
 * so the limit still applies there and large repositories keep the fetch savings
 * it was added for.
 */
