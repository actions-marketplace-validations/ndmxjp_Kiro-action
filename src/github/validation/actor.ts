/**
 * Check that the workflow was triggered by a human actor.
 * Prevents automated tools or bots from looping Kiro back on itself.
 */

import type { Octokit } from "@octokit/rest";
import type { GitHubContext } from "../context";
import { isAllowedBot } from "./permissions";
...
export async function checkHumanActor(
  ...
  // allow-list only ever applies to non-User accounts. Some app actors are not
  // resolvable via the Users API and 404 — that path is handled in the catch.
  ...
  // consulted here; write-access enforcement happens in checkWritePermissions.
